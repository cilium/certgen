...
func generateCertificates() error {
	...
	for i, cfg := range certConfigs.Certs {
		log.With(
			slog.String(logfields.K8sSecretName, cfg.Name),
			slog.String(logfields.K8sSecretNamespace, cfg.Namespace),
	...
	for _, cert := range certs {
		log.With(
			slog.String(logfields.K8sSecretName, cert.Name),
			slog.String(logfields.K8sSecretNamespace, cert.Namespace),
